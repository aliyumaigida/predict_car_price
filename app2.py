...
mileage_kmpl = st.number_input("Mileage (kmpl)", min_value=5., max_value=35.0, step=0.1)
engine_cc = st.number_input("Engine (cc)", min_value=0, max_value=5000)
owner_count = st.number_input("Number of Previous Owners", min_value=0)
accidents_reported = st.number_input("Accidents Reported", min_value=0)

fuel_type = st.selectbox("Fuel Type", ["Petrol", "Diesel", "Electric", "CNG", "Hybrid"])
brand = st.selectbox("Brand", ["Chevrolet", "Honda", "BMW", "Hyundai", "Nissan", "Tesla",
